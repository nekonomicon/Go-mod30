...
void ClientCommand( edict_t *pEntity )
{
    ...
    else if ( FStrEq(pcmd, "duplicatemode" ) )
    {
		...
		pPlayer->r_duplicatemode = 1;
		pPlayer->r_removemode = 0;
		pPlayer->r_rendermode = 0;
		pPlayer->r_cameramode = 0;
		pPlayer->r_gibmode = 0;
		pPlayer->r_posermode = 0;
    ...
    else if ( FStrEq(pcmd, "removemode" ) )
    {
		...
		pPlayer->r_duplicatemode = 0;
		pPlayer->r_removemode = 1;
		pPlayer->r_rendermode = 0;
		pPlayer->r_cameramode = 0;
		pPlayer->r_gibmode = 0;
		pPlayer->r_posermode = 0;
	...
	//  CameraMode
    else if ( FStrEq(pcmd, "cameramode" ) )
    {
       	CBasePlayer * pPlayer = GetClassPtr((CBasePlayer *)pev);
		pPlayer->r_duplicatemode = 0;
		pPlayer->r_removemode = 0;
		pPlayer->r_rendermode = 0;
		pPlayer->r_cameramode = 1;
		pPlayer->r_gibmode = 0;
		pPlayer->r_posermode = 0;
    ...
    else if ( FStrEq(pcmd, "posermode" ) )
    {
		...
		pPlayer->r_cameramode = 0;
		pPlayer->r_gibmode = 0;
		pPlayer->r_posermode = 1;
    }
    else if ( FStrEq(pcmd, "+noclip" ) )
    {
int allow_noclip = CVAR_GET_FLOAT("allow_noclip");
 if (allow_noclip == 1)
{
	CBasePlayer * pPlayer = GetClassPtr((CBasePlayer *)pev);
	...
	int allow_props = CVAR_GET_FLOAT("allow_props");
 if (allow_props == 1)
{

	// GIVE CUSTOM1
    if ( FStrEq(pcmd, "custom1_button" ) )
    {
        
		UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
		CBaseEntity::Create("custom1", pev->origin + gpGlobals->v_forward * 128, pev->angles);
		...
		// GIVE CUSTOM2
    if ( FStrEq(pcmd, "custom2_button" ) )
    {
        
		UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
		CBaseEntity::Create("custom2", pev->origin + gpGlobals->v_forward * 128, pev->angles);
		EMIT_SOUND(ENT(pev), CHAN_VOICE, "button.wav", 0.94, ATTN_NORM);
    }
		...
		// GIVE CUSTOM3
    if ( FStrEq(pcmd, "custom3_button" ) )
    {
        
		UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
		CBaseEntity::Create("custom3", pev->origin + gpGlobals->v_forward * 128, pev->angles);
		EMIT_SOUND(ENT(pev), CHAN_VOICE, "button.wav", 0.94, ATTN_NORM);
    }


		// GIVE CUSTOM4
    if ( FStrEq(pcmd, "custom4_button" ) )
    {
        
		UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
		CBaseEntity::Create("custom4", pev->origin + gpGlobals->v_forward * 128, pev->angles);
		EMIT_SOUND(ENT(pev), CHAN_VOICE, "button.wav", 0.94, ATTN_NORM);
    }


		// GIVE CUSTOM5
    if ( FStrEq(pcmd, "custom5_button" ) )
    {
        
		UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
		CBaseEntity::Create("custom5", pev->origin + gpGlobals->v_forward * 128, pev->angles);
		EMIT_SOUND(ENT(pev), CHAN_VOICE, "button.wav", 0.94, ATTN_NORM);
    }

		// GIVE CUSTOM6
    if ( FStrEq(pcmd, "custom6_button" ) )
    {
        
		UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
		CBaseEntity::Create("custom6", pev->origin + gpGlobals->v_forward * 128, pev->angles);
		EMIT_SOUND(ENT(pev), CHAN_VOICE, "button.wav", 0.94, ATTN_NORM);
    }
	...
	// NEW PROPS - RANGER366
		...
		// GIVE CUSTOM7
    if ( FStrEq(pcmd, "custom7_button" ) )
    {
        
		UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
		CBaseEntity::Create("custom7", pev->origin + gpGlobals->v_forward * 128, pev->angles);
		EMIT_SOUND(ENT(pev), CHAN_VOICE, "button.wav", 0.94, ATTN_NORM);
    }
		...
		// GIVE CUSTOM8
    if ( FStrEq(pcmd, "custom8_button" ) )
    {
        
		UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
		CBaseEntity::Create("custom8", pev->origin + gpGlobals->v_forward * 128, pev->angles);
		EMIT_SOUND(ENT(pev), CHAN_VOICE, "button.wav", 0.94, ATTN_NORM);
    }
		...
		// GIVE CUSTOM9
    if ( FStrEq(pcmd, "custom9_button" ) )
    {
        
		UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
		CBaseEntity::Create("custom9", pev->origin + gpGlobals->v_forward * 128, pev->angles);
		EMIT_SOUND(ENT(pev), CHAN_VOICE, "button.wav", 0.94, ATTN_NORM);
    }


		// GIVE CUSTOM10
    if ( FStrEq(pcmd, "custom10_button" ) )
    {
        
		UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
		CBaseEntity::Create("custom10", pev->origin + gpGlobals->v_forward * 128, pev->angles);
		EMIT_SOUND(ENT(pev), CHAN_VOICE, "button.wav", 0.94, ATTN_NORM);
    }


		// GIVE CUSTOM11
    if ( FStrEq(pcmd, "custom11_button" ) )
    {
        
		UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
		CBaseEntity::Create("custom11", pev->origin + gpGlobals->v_forward * 128, pev->angles);
		EMIT_SOUND(ENT(pev), CHAN_VOICE, "button.wav", 0.94, ATTN_NORM);
    }

		// GIVE CUSTOM12
    if ( FStrEq(pcmd, "custom12_button" ) )
    {
        
		UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
		CBaseEntity::Create("custom12", pev->origin + gpGlobals->v_forward * 128, pev->angles);
		EMIT_SOUND(ENT(pev), CHAN_VOICE, "button.wav", 0.94, ATTN_NORM);
    }
	...
	//NEW PROPS END
		...
		int allow_monsterspawn = CVAR_GET_FLOAT("allow_monsterspawn");
 if (allow_monsterspawn == 1)
{


	 // SPAWN HGRUNT
    if ( FStrEq(pcmd, "hgrunt_button" ) )
    {
	
        
		UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
		CBaseEntity::Create("monster_human_grunt", pev->origin + gpGlobals->v_forward * 128, pev->angles,pev->owner);
		EMIT_SOUND(ENT(pev), CHAN_VOICE, "button.wav", 0.94, ATTN_NORM);
		
		//CBaseDelay::Create("trigger_camera",
    ...
    {
		int allow_bigmonsters = CVAR_GET_FLOAT("allow_bigmonsters");
        if (allow_bigmonsters == 1)
		{
    ...
    else if ( FStrEq(pcmd, "gman_button" ) )
    {
		...
		UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
		CBaseEntity::Create("monster_gman", pev->origin + gpGlobals->v_forward * 128, pev->angles);
			EMIT_SOUND(ENT(pev), CHAN_VOICE, "button.wav", 0.94, ATTN_NORM);
    }
    ...
    else if ( FStrEq(pcmd, "otis_button" ) )
    {
		...
		UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
		CBaseEntity::Create("monster_otis", pev->origin + gpGlobals->v_forward * 128, pev->angles);
			EMIT_SOUND(ENT(pev), CHAN_VOICE, "button.wav", 0.94, ATTN_NORM);
    }
	...
	int allow_weaponspawn = CVAR_GET_FLOAT("allow_weaponspawn");
 if (allow_weaponspawn == 1)
{

		// GIVE 9mmhandgun
    if ( FStrEq(pcmd, "9mm_button" ) )
    {
    ...
    {
  int allow_toolgun = CVAR_GET_FLOAT("allow_toolgun");
 if (allow_toolgun == 1)
{
	edict_t	*pent;
	...
	// GIVE AK47
    else if ( FStrEq(pcmd, "csknife_button" ) )
    {
       
		
		edict_t	*pent;
		int istr = MAKE_STRING("weapon_punch");
	...
	if ( FStrEq(pcmd, "snark" ) )
    {
		...
		edict_t	*pent;
		int istr = MAKE_STRING("weapon_snark");
	...
	if ( FStrEq(pcmd, "tripmine" ) )
    {
		...
		edict_t	*pent;
		int istr = MAKE_STRING("weapon_tripmine");
...
int allow_voices = CVAR_GET_FLOAT("allow_voices");
 if (allow_voices == 1)
{

    if ( FStrEq(pcmd, "voice1_say" ) )
    {
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "voice/hey.wav", 1.0, ATTN_NONE, 0, 100);
	}

	else if ( FStrEq(pcmd, "voice2_say" ) )
    {
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "voice/yo.wav", 1.0, ATTN_NONE, 0, 100);
	}

	else if ( FStrEq(pcmd, "voice3_say" ) )
    {
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "voice/there_he_is.wav", 1.0, ATTN_NONE, 0, 100);
	}

	else if ( FStrEq(pcmd, "voice4_say" ) )
    {
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "voice/target_located.wav", 1.0, ATTN_NONE, 0, 100);
	}

	else if ( FStrEq(pcmd, "voice5_say" ) )
    {
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "voice/what_the.wav", 1.0, ATTN_NONE, 0, 100);
	}

	else if ( FStrEq(pcmd, "voice6_say" ) )
    {
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "voice/gogogo.wav", 1.0, ATTN_NONE, 0, 100);
	}

	else if ( FStrEq(pcmd, "voice7_say" ) )
    {
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "voice/female_fire_death.wav", 1.0, ATTN_NONE, 0, 100);
	}

	else if ( FStrEq(pcmd, "voice8_say" ) )
    {
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "voice/fire_death_scream.wav", 1.0, ATTN_NONE, 0, 100);
	}



	 if ( FStrEq(pcmd, "voice9_say" ) )
    {
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "barney/dobettertogether.wav", 1.0, ATTN_NONE, 0, 100);
	}

	else if ( FStrEq(pcmd, "voice10_say" ) )
    {
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "barney/imwithyou.wav", 1.0, ATTN_NONE, 0, 100);
	}

	else if ( FStrEq(pcmd, "voice11_say" ) )
    {
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "barney/teamup1.wav", 1.0, ATTN_NONE, 0, 100);
	}

	else if ( FStrEq(pcmd, "voice12_say" ) )
    {
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "barney/gladtolendhand.wav", 1.0, ATTN_NONE, 0, 100);
	}

	else if ( FStrEq(pcmd, "voice13_say" ) )
    {
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "barney/letsgo.wav", 1.0, ATTN_NONE, 0, 100);
	}

	else if ( FStrEq(pcmd, "voice14_say" ) )
    {
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "barney/letsmoveit.wav", 1.0, ATTN_NONE, 0, 100);
	}

	else if ( FStrEq(pcmd, "voice15_say" ) )
    {
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "barney/helpothers.wav", 1.0, ATTN_NONE, 0, 100);
	}

	else if ( FStrEq(pcmd, "voice16_say" ) )
    {
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "barney/iwaithere.wav", 1.0, ATTN_NONE, 0, 100);
	}

	else if ( FStrEq(pcmd, "voice17_say" ) )
    {
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "barney/slowingyoudown.wav", 1.0, ATTN_NONE, 0, 100);
	}

	else if ( FStrEq(pcmd, "voice18_say" ) )
    {
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "barney/illwait.wav", 1.0, ATTN_NONE, 0, 100);
	}

	else if ( FStrEq(pcmd, "voice19_say" ) )
    {
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "barney/seeya.wav", 1.0, ATTN_NONE, 0, 100);
	}

	else if ( FStrEq(pcmd, "voice20_say" ) )
    {
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "barney/standguard.wav", 1.0, ATTN_NONE, 0, 100);
	}
...
PRECACHE_SOUND("barney/dobettertogether.wav");
PRECACHE_SOUND("barney/imwithyou.wav");
PRECACHE_SOUND("barney/teamup1.wav");
PRECACHE_SOUND("barney/gladtolendhand.wav");
PRECACHE_SOUND("barney/letsgo.wav");
PRECACHE_SOUND("barney/letsmoveit.wav");
PRECACHE_SOUND("barney/helpothers.wav");
PRECACHE_SOUND("barney/iwaithere.wav");
PRECACHE_SOUND("barney/slowingyoudown.wav");
PRECACHE_SOUND("barney/illwait.wav");
PRECACHE_SOUND("barney/seeya.wav");
PRECACHE_SOUND("barney/standguard.wav");
...
// player pain sounds
	...
	// PRECACHE ALL MONSTERS FOR GOMOD

	int precache_all = CVAR_GET_FLOAT("precache_all_npcs");
	...
	if(allow_monsterspawn == 1)
	{
		...
		UTIL_PrecacheOther("custom1");
		UTIL_PrecacheOther("custom2");
		UTIL_PrecacheOther("custom3");
		UTIL_PrecacheOther("custom4");
		UTIL_PrecacheOther("custom5");
		UTIL_PrecacheOther("custom6");
		UTIL_PrecacheOther("custom7");
		UTIL_PrecacheOther("custom8");
		UTIL_PrecacheOther("custom9");
		UTIL_PrecacheOther("custom10");
		UTIL_PrecacheOther("custom11");
		UTIL_PrecacheOther("custom12");
	}

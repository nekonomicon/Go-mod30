...
Vector CBaseEntity::FireBulletsPhysGun( ULONG cShots, Vector vecSrc, Vector vecDirShooting, Vector vecSpread, float flDistance, int iBulletType, int iTracerFreq, int iDamage, entvars_t *pevAttacker, int shared_rand )
{
	...
	UTIL_TraceLine(vecSrc, vecEnd, dont_ignore_monsters, ENT(pev)/*pentIgnore*/, &tr);
		...
		if ( pEntity )
		{
				...
				{
				//	return Vector( x * vecSpread.x, y * vecSpread.y, 0.0 );
						int allow_bspmove = CVAR_GET_FLOAT("bspmove");
					if (allow_bspmove == 1)
					{
...
Vector CBaseEntity::FireBulletsDuplicator( ULONG cShots, Vector vecSrc, Vector vecDirShooting, Vector vecSpread, float flDistance, int iBulletType, int iTracerFreq, int iDamage, entvars_t *pevAttacker, int shared_rand )
{
	...
	for ( ULONG iShot = 1; iShot <= cShots; iShot++ )
	{
		...
		UTIL_TraceLine(vecSrc, vecEnd, dont_ignore_monsters, ENT(pev)/*pentIgnore*/, &tr);
			...
			if(	monster_type == 23)//DUPLICATE HUMAN_MASSN
			{
				UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
				CBaseEntity::Create("monster_human_massn",tr.vecEndPos, pev->angles);
			}


			if(	monster_type == 24)//DUPLICATE OTIS
			{
				UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
				CBaseEntity::Create("monster_otis",tr.vecEndPos, pev->angles);
			}


			if(	monster_type == 25)//DUPLICATE PITDRONE
			{
				UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
				CBaseEntity::Create("monster_pit_drone",tr.vecEndPos, pev->angles);
			}



			if(	monster_type == 26)//DUPLICATE ichthyosaur
			{
				UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
				CBaseEntity::Create("monster_ichthyosaur",tr.vecEndPos, pev->angles);
			}
		
		
			if(	monster_type == 27)//DUPLICATE HL2 ZOMBIE
			{
				UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
				CBaseEntity::Create("monster_zombie_hl2",tr.vecEndPos, pev->angles);
			}




			if(	monster_type == 28)//DUPLICATE GONOME
			{
				UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
				CBaseEntity::Create("monster_gonome",tr.vecEndPos, pev->angles);
			}



			if(	monster_type == 30)//DUPLICATE CUSTOM1 PROP
			{
				UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
				CBaseEntity::Create("custom1",tr.vecEndPos, pev->angles);
			}


			if(	monster_type == 31)//DUPLICATE CUSTOM2 PROP
			{
				UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
				CBaseEntity::Create("custom2",tr.vecEndPos, pev->angles);
			}


			if(	monster_type == 32)//DUPLICATE CUSTOM3 PROP
			{
				UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
				CBaseEntity::Create("custom3",tr.vecEndPos, pev->angles);
			}


			if(	monster_type == 33)//DUPLICATE CUSTOM4 PROP
			{
				UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
				CBaseEntity::Create("custom4",tr.vecEndPos, pev->angles);
			}

			if(	monster_type == 34)//DUPLICATE CUSTOM5 PROP
			{
				UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
				CBaseEntity::Create("custom5",tr.vecEndPos, pev->angles);
			}

			if(	monster_type == 35)//DUPLICATE CUSTOM6 PROP
			{
				UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
				CBaseEntity::Create("custom6",tr.vecEndPos, pev->angles);
			}
			if(	monster_type == 36)//DUPLICATE CUSTOM7 PROP
			{
				UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
				CBaseEntity::Create("custom7",tr.vecEndPos, pev->angles);
			}


			if(	monster_type == 37)//DUPLICATE CUSTOM8 PROP
			{
				UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
				CBaseEntity::Create("custom8",tr.vecEndPos, pev->angles);
			}


			if(	monster_type == 38)//DUPLICATE CUSTOM9 PROP
			{
				UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
				CBaseEntity::Create("custom9",tr.vecEndPos, pev->angles);
			}


			if(	monster_type == 39)//DUPLICATE CUSTOM10 PROP
			{
				UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
				CBaseEntity::Create("custom10",tr.vecEndPos, pev->angles);
			}

			if(	monster_type == 40)//DUPLICATE CUSTOM11 PROP
			{
				UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
				CBaseEntity::Create("custom11",tr.vecEndPos, pev->angles);
			}

			if(	monster_type == 41)//DUPLICATE CUSTOM12 PROP
			{
				UTIL_MakeVectors( Vector( 0, pev->v_angle.y, 0 ) );
				CBaseEntity::Create("custom12",tr.vecEndPos, pev->angles);
			}
...
Vector CBaseEntity::FireBulletsDuplicatorSelect( ULONG cShots, Vector vecSrc, Vector vecDirShooting, Vector vecSpread, float flDistance, int iBulletType, int iTracerFreq, int iDamage, entvars_t *pevAttacker, int shared_rand )
{
	...
	pEntity = FindEntityForwardNew( this );
		if ( pEntity )
		{
			...
			if( STRING(pEntity->pev->classname) == "monster_human_massn") //CHECK FOR HUMAN_MASSN
			{
				monster_type = 23;
			...
			if( STRING(pEntity->pev->classname) == "monster_otis") //CHECK FOR OTIS
			{
				monster_type = 24;
			...
			if( STRING(pEntity->pev->classname) == "monster_pit_drone") //CHECK FOR PITDRONE
			{
				monster_type = 25;
			...
			if( STRING(pEntity->pev->classname) == "monster_ichthyosaur") //CHECK FOR ichthyosaur
			{
				monster_type = 26;
			...
			if( STRING(pEntity->pev->classname) == "monster_zombie_hl2") //CHECK FOR HL2 ZOMBIE
			{
				monster_type = 27;
			...
			if( STRING(pEntity->pev->classname) == "monster_gonome") //CHECK FOR GONOME
			{
				monster_type = 28;
			...
			if( STRING(pEntity->pev->classname) == "monster_hl2headcrab") //CHECK FOR HL2 HEADCRAB
			{
				monster_type = 29;
			...
			if( STRING(pEntity->pev->classname) == "custom1") //CHECK FOR CUSTOM1 PROP
			{
				monster_type = 30;
			
			}

			if( STRING(pEntity->pev->classname) == "custom2") //CHECK FOR CUSTOM2 PROP
			{
				monster_type = 31;
			...
			if( STRING(pEntity->pev->classname) == "custom3") //CHECK FOR CUSTOM3 PROP
			{
				monster_type = 32;
			...
			if( STRING(pEntity->pev->classname) == "custom4") //CHECK FOR CUSTOM4 PROP
			{
				monster_type = 33;
			...
			if( STRING(pEntity->pev->classname) == "custom5") //CHECK FOR CUSTOM5 PROP
			{
				monster_type = 34;
			...
			if( STRING(pEntity->pev->classname) == "custom6") //CHECK FOR CUSTOM6 PROP
			{
				monster_type = 35;
			
			}

			if( STRING(pEntity->pev->classname) == "custom7") //CHECK FOR CUSTOM1 PROP
			{
				monster_type = 36;
			
			}

			if( STRING(pEntity->pev->classname) == "custom8") //CHECK FOR CUSTOM2 PROP
			{
				monster_type = 37;
			...
			if( STRING(pEntity->pev->classname) == "custom9") //CHECK FOR CUSTOM3 PROP
			{
				monster_type = 38;
			...
			if( STRING(pEntity->pev->classname) == "custom10") //CHECK FOR CUSTOM4 PROP
			{
				monster_type = 39;
			...
			if( STRING(pEntity->pev->classname) == "custom11") //CHECK FOR CUSTOM5 PROP
			{
				monster_type = 40;
			...
			if( STRING(pEntity->pev->classname) == "custom12") //CHECK FOR CUSTOM6 PROP
			{
				monster_type = 41;
...
Vector CBaseEntity::FireBulletsPoserTool( ULONG cShots, Vector vecSrc, Vector vecDirShooting, Vector vecSpread, float flDistance, int iBulletType, int iTracerFreq, int iDamage, entvars_t *pevAttacker, int shared_rand )
{
	...
	for ( ULONG iShot = 1; iShot <= cShots; iShot++ )
	{
		...
		if ( pEntity )
		{
			...
			//	Animation.ResetSequenceInfo();
				pEntity->pev->frame = frame;
			//	pEntity->Think();
			//	Animation.StudioFrameAdvance ( 0.1 );

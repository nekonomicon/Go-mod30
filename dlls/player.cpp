...
// many features in this function use v_forward, so makevectors now.
	...
	EMIT_SOUND(ENT(pev), CHAN_VOICE, "player/1997jumpon1.wav", 1, ATTN_NORM);
...
CBaseEntity *FindEntityForwardNew1( CBaseEntity *pMe )
{
	TraceResult tr;
...
// fallpain sound, and damage will be inflicted based on how far the player fell
	...
	if (FBitSet(pev->flags, FL_ONGROUND))
	{		
		if (m_flFallVelocity > 45 && !g_pGameRules->IsMultiplayer())
		{
			CSoundEnt::InsertSound ( bits_SOUND_PLAYER, pev->origin, m_flFallVelocity, 0.2 );
			// ALERT( at_console, "fall %f\n", m_flFallVelocity );
			EMIT_SOUND(ENT(pev), CHAN_VOICE, "player/1997jumpon2.wav", 1, ATTN_NORM);
		}

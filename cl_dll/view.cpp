...
// Rangers bobside code, this fixes jitters in the mutliplayer since the clock (pparams->time) isn't quite linear
float V_CalcBobSide ( struct ref_params_s *pparams )
{
	static	double	bobtime;
	...
	bobtime += pparams->frametime;
	cycle = bobtime - (int)( bobtime / cl_bobsidecycle->value ) * cl_bobsidecycle->value;
	...
	bobside = sqrt( vel[0] * vel[0] + vel[1] * vel[1] ) * cl_bobside->value;
	bobside = bobside * 0.3 + bobside * 0.7 * sin(cycle);
	bobside = min( bobside, 4 );
	bobside = max( bobside, -7 );
	return bobside;
...
//	V_CalcNewBob ( pparams );
	...
	//	view->origin[ i ] += bob * 0.3 * pparams->right[ i ];
	//	view->origin[ i ] += pparams->up[i]*bob*0.2;
		view->origin[i] += pparams->right[i]*bobside*0.3;
		view->origin[i] += pparams->up[i]*bob*0.3;
...
void V_Init (void)
{
	...
	cl_bobcycle			= gEngfuncs.pfnRegisterVariable( "cl_bobcycle","0.43", 0 );// best default for my experimental gun wag (sjb)
	cl_bob				= gEngfuncs.pfnRegisterVariable( "cl_bob","0.007", 0 );// best default for my experimental gun wag (sjb)
	cl_bobup			= gEngfuncs.pfnRegisterVariable( "cl_bobup","0.45", 0 );

	cl_bobsidecycle			= gEngfuncs.pfnRegisterVariable( "cl_bobsidecycle","0.86", 0 );// best default for my experimental gun wag (sjb)
	cl_bobside				= gEngfuncs.pfnRegisterVariable( "cl_bobside","0.013", 0 );// best default for my experimental gun wag (sjb)
	cl_bobsideup			= gEngfuncs.pfnRegisterVariable( "cl_bobsideup","0.5", 0 );
